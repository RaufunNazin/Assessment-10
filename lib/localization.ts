export const translations = {
  en: {
    // Navigation
    nav: {
      courses: "Courses",
      skills: "Skills",
      admissions: "Admissions",
      jobs: "Jobs",
      more: "More",
      login: "Login",
      signup: "Sign Up",
    },
    // Hero Section
    hero: {
      title: "IELTS Course by Munzereen Shahid",
      rating: "positive reviews",
      description:
        "Complete IELTS preparation course with expert guidance from Munzereen Shahid. Master all four skills: Reading, Writing, Listening, and Speaking with proven strategies and comprehensive practice materials.",
    },
    // Course Banner
    banner: {
      freePdf: "Free PDF",
      title: "IELTS Confirm 7+ Score (Guideline)",
      description:
        "Best strategies to achieve your target IELTS score with expert guidelines.",
      downloadBtn: "Download Free PDF",
    },
    // What You'll Learn
    learn: {
      title: "What You'll Learn",
      points: [
        {
          title:
            "Complete preparation and answering strategies for all IELTS sections",
          description:
            "Master Reading, Writing, Listening, and Speaking with proven techniques.",
        },
        {
          title: "IELTS Writing Task 1 & 2 complete structure and essay types",
          description:
            "Detailed writing strategies for both Academic and General Training modules.",
        },
        {
          title:
            "Advanced speaking techniques with power words and presentation skills",
          description:
            "Learn to speak confidently in the IELTS Speaking test with advanced vocabulary.",
        },
        {
          title:
            "Time management strategies for achieving better scores during the exam",
          description:
            "Master time management and exam strategies to reach your target score.",
        },
        {
          title: "Complete understanding of IELTS exam rules and regulations",
          description:
            "Clear knowledge of all IELTS rules and marking criteria.",
        },
        {
          title: "Mock tests and practice experience for Reading and Listening",
          description:
            "Real exam experience through practice tests with detailed feedback.",
        },
      ],
    },
    // Course Details
    details: {
      title: "Course Details",
      whyTakeTitle: "Why Take This IELTS Course",
      features: {
        videos: "Video Lectures",
        duration: "Course Duration",
        students: "Enrolled Students",
        certificates: "Certificates",
        lifetime: "Lifetime",
      },
      modules: [
        {
          title: "Introduction",
          lessons: [
            "Video: IELTS Introduction",
            "Video: IELTS Overview",
            "Video: How to Prepare for IELTS?",
            "Video: Making a Daily Routine",
          ],
        },
        {
          title: "IELTS Reading",
          lessons: [
            "Reading Strategies and Techniques",
            "Different Question Types",
            "Time Management for Reading",
            "Practice Tests with Solutions",
          ],
        },
        {
          title: "IELTS Writing",
          lessons: [
            "Task 1: Academic Writing (Graphs, Charts, Diagrams)",
            "Task 1: General Training (Letters)",
            "Task 2: Essay Writing Techniques",
            "Common Writing Mistakes to Avoid",
          ],
        },
        {
          title: "IELTS Listening",
          lessons: [
            "Listening Strategies",
            "Note-taking Techniques",
            "Different Accent Recognition",
            "Mock Tests and Practice",
          ],
        },
        {
          title: "IELTS Speaking",
          lessons: [
            "Part 1: Introduction and Interview",
            "Part 2: Long Turn (Cue Card)",
            "Part 3: Discussion",
            "Fluency and Pronunciation Tips",
          ],
        },
      ],
    },
    // Sidebar
    sidebar: {
      price: "৳1000",
      enrollBtn: "Enroll Now",
      whatYouGet: "What you'll get:",
      features: [
        "Course for 3000+ students",
        "Lifetime access",
        "50+ video lectures",
        "10+ assignments and practice tests",
        "20+ certificates",
        "Mobile app access",
        "Expert support",
        "Community access",
      ],
    },
    // FAQ
    faq: {
      title: "Frequently Asked Questions",
      questions: [
        {
          question: "What is the duration of this IELTS course?",
          answer:
            "This is a self-paced course with lifetime access. You can complete it according to your schedule, typically taking 2-3 months for thorough preparation.",
        },
        {
          question:
            "Is this course suitable for both Academic and General Training IELTS?",
          answer:
            "Yes, this course covers both Academic and General Training IELTS modules with specific strategies and practice materials for each.",
        },
        {
          question: "Will I get a certificate after completing the course?",
          answer:
            "Yes, you will receive a certificate of completion from 10 Minute School after finishing all course modules.",
        },
        {
          question: "Can I access the course on mobile devices?",
          answer:
            "The course is fully optimized for mobile devices and you can access it through our mobile app.",
        },
        {
          question: "Is there any support available if I have questions?",
          answer:
            "Yes, we provide expert support through our discussion forums and you can ask questions anytime during the course.",
        },
      ],
    },
    // Reviews
    reviews: {
      title: "Student Success Stories",
      subtitle: "See what our students achieved",
    },
    // Footer
    footer: {
      company: {
        title: "Company",
        links: ["About Us", "Careers", "Press", "Blog", "Contact"],
      },
      courses: {
        title: "Courses",
        links: [
          "IELTS",
          "Spoken English",
          "Email Marketing",
          "Complete Grammar",
          "Academic Writing",
        ],
      },
      support: {
        title: "Support",
        links: [
          "Help Center",
          "Community",
          "Live Chat",
          "FAQ",
          "Contact Support",
        ],
      },
      legal: {
        title: "Legal",
        links: [
          "Privacy Policy",
          "Terms of Service",
          "Cookie Policy",
          "Refund Policy",
        ],
      },
      contact: {
        title: "Contact Information",
        phone: "Call us: 16910 (24x7)",
        whatsapp: "WhatsApp: +8801896016252 (24x7)",
        email: "Email: support@10minuteschool.com",
        address: "House 25, Road 5, Dhanmondi, Dhaka-1205",
      },
      social: {
        title: "Follow Us",
        copyright: "© 2024 10 Minute School. All rights reserved.",
        description:
          "Bangladesh's largest online education platform with millions of students.",
      },
    },
  },
  bn: {
    // Navigation
    nav: {
      courses: "কোর্সসমূহ",
      skills: "স্কিলস",
      admissions: "ভর্তি",
      jobs: "চাকরি",
      more: "আরও",
      login: "লগইন",
      signup: "সাইন আপ",
    },
    // Hero Section
    hero: {
      title: "IELTS Course by Munzereen Shahid",
      rating: "শিক্ষার্থী কোর্স ও রেটিং নিয়েছেন",
      description:
        "Academic IELTS এবং General Training IELTS- এর কমপ্লিট প্রিপারেশনের জন্য একটি কোর্সই যথেষ্ট। IELTS Instructor এর পাইওনিয়ার আপনার সাফল্যের সাথে কোর্সটি করুন এবং ভালো জানুন কোন আপনার IELTS Course-এ।",
    },
    // Course Banner
    banner: {
      freePdf: "ফ্রি PDF",
      title: "IELTS Confirm 7+ Score (Guideline)",
      description:
        "IELTS আসার score করার সেরা Strategies আপনার সেবায় গাইডলাইনে।",
      downloadBtn: "ফ্রি PDF Download করুন",
    },
    // What You'll Learn
    learn: {
      title: "কোর্সটি করে যা শিখবেন",
      points: [
        {
          title:
            "IELTS পরীক্ষার প্রতিটি সেকশনের সম্পূর্ণ প্রস্তুতি ও উত্তরের কৌশল",
          description:
            "Reading, Writing, Listening এবং Speaking - চারটি সেকশনেই দক্ষতা অর্জন করুন।",
        },
        {
          title:
            "IELTS Writing Task 1 ও IELTS Writing Task 2 এর কেস স্টাডি কোর্স পেতে সম্পূর্ণ Structure ও Essay type",
          description:
            "Academic ও General Training উভয় মডিউলের জন্য বিস্তারিত লেখার কৌশল।",
        },
        {
          title:
            "IELTS Speaking test-এ Advanced/Power Words ব্যবহার করে যেভাবে চমৎকার নিজেকে উপস্থাপন করা যায় বলার পদ্ধতি",
          description:
            "Speaking পরীক্ষায় আত্মবিশ্বাসের সাথে উত্তর দেওয়ার কৌশল শিখুন।",
        },
        {
          title:
            "IELTS পরীক্ষা চলাকালীন বিশেষভাবে সময়ের সঠিক ব্যবহারের মাধ্যমে ভালো কোর্স অর্জনের কৌশল",
          description:
            "Time management এবং exam strategy যা আপনাকে target score এ পৌঁছাতে সাহায্য করবে।",
        },
        {
          title:
            "সেরা IELTS প্রস্তুতি দিতে প্রতিটি পরীক্ষার নিয়ম-আবদ্ধতা বিষয়ক নিয়মাবলী নিয়ে বিস্তারিত ধারণা",
          description:
            "IELTS পরীক্ষার সকল নিয়মকানুন এবং marking criteria সম্পর্কে স্পষ্ট ধারণা।",
        },
        {
          title:
            "IELTS Reading এবং IELTS Listening Mock Test এর মাধ্যমে IELTS পরীক্ষার বিভিন্ন এক্সপেরিয়েন্স ও Band Score সম্পর্কে পরিপূর্ণ ধারণা",
          description:
            "Real exam experience এর মতো practice test এবং detailed feedback।",
        },
      ],
    },
    // Course Details
    details: {
      title: "কোর্স সম্পর্কে বিস্তারিত",
      whyTakeTitle: "IELTS Course-টি কেন সম্পদে",
      features: {
        videos: "ভিডিও লেকচার",
        duration: "কোর্সের মেয়াদ",
        students: "এনরোলড স্টুডেন্ট",
        certificates: "সার্টিফিকেট",
        lifetime: "লাইফটাইম",
      },
      modules: [
        {
          title: "Introduction",
          lessons: [
            "Video: IELTS: Introduction",
            "Video: IELTS: Overview",
            "Video: How to Prepare for IELTS?",
            "Video: Making a Daily Routine",
          ],
        },
        {
          title: "Academic Reading",
          lessons: [
            "Reading Strategies এবং Techniques",
            "বিভিন্ন Question Types",
            "Reading এর জন্য Time Management",
            "Practice Tests এবং Solutions",
          ],
        },
        {
          title: "Academic Writing",
          lessons: [
            "Task 1: Academic Writing (Graphs, Charts, Diagrams)",
            "Task 2: Essay Writing Techniques",
            "Common Writing Mistakes এড়ানোর উপায়",
            "Writing Band Score বৃদ্ধির কৌশল",
          ],
        },
        {
          title: "Listening",
          lessons: [
            "Listening Strategies",
            "Note-taking Techniques",
            "বিভিন্ন Accent চেনার কৌশল",
            "Mock Tests এবং Practice",
          ],
        },
        {
          title: "Speaking",
          lessons: [
            "Part 1: Introduction এবং Interview",
            "Part 2: Long Turn (Cue Card)",
            "Part 3: Discussion",
            "Fluency এবং Pronunciation Tips",
          ],
        },
      ],
    },
    // Sidebar
    sidebar: {
      price: "৳১০০০",
      enrollBtn: "কোর্সটি কিনুন",
      whatYouGet: "এই কোর্সে যা থাকছে:",
      features: [
        "কোর্সটি করেছেন ৩০০০+ জন",
        "সময় লাগবে ৫০ ঘন্টা",
        "৫০টি ভিডিও লেকচার",
        "১০টি লিখিত এবং ১০টি শিলনীর মক টেস্ট",
        "২০টি লেকচার শিট",
        "১টি ফ্রি হ্যান্ডনোট বই",
        "ফেসবুক সাপোর্ট গ্রুপ",
        "লাইফটাইম এক্সেস",
      ],
    },
    // FAQ
    faq: {
      title: "সচরাচর জিজ্ঞাসা",
      questions: [
        {
          question: "কোর্সটি কিনে কিভাবে করবো?",
          answer:
            "কোর্স কেনার পর আপনি আপনার ড্যাশবোর্ডে কোর্সটি পাবেন। সেখান থেকে যেকোনো সময় ভিডিও দেখে কোর্সটি সম্পন্ন করতে পারবেন।",
        },
        {
          question: "কোর্সটি কি Academic এবং General Training দুটোর জন্যই?",
          answer:
            "হ্যাঁ, এই কোর্সে Academic এবং General Training দুটো মডিউলের জন্যই আলাদা আলাদা কৌশল এবং প্র্যাকটিস ম্যাটেরিয়াল রয়েছে।",
        },
        {
          question: "কোর্স শেষে কি সার্টিফিকেট পাবো?",
          answer:
            "হ্যাঁ, কোর্সের সব মডিউল সম্পন্ন করার পর আপনি ১০ মিনিট স্কুলের পক্ষ থেকে একটি সার্টিফিকেট পাবেন।",
        },
        {
          question: "মোবাইল দিয়ে কি কোর্সটি করা যাবে?",
          answer:
            "অবশ্যই! কোর্সটি সম্পূর্ণভাবে মোবাইল ফ্রেন্ডলি এবং আমাদের মোবাইল অ্যাপের মাধ্যমেও এক্সেস করতে পারবেন।",
        },
        {
          question: "কোনো সমস্যা হলে সাপোর্ট পাবো?",
          answer:
            "হ্যাঁ, আমাদের এক্সপার্ট সাপোর্ট টিম এবং ডিসকাশন ফোরামের মাধ্যমে যেকোনো সময় সাহায্য পেতে পারেন।",
        },
      ],
    },
    // Reviews
    reviews: {
      title: "শিক্ষার্থীদের সাফল্যের গল্প",
      subtitle: "দেখুন আমাদের শিক্ষার্থীরা কী অর্জন করেছেন",
    },
    // Footer
    footer: {
      company: {
        title: "কোম্পানি",
        links: ["আমাদের সম্পর্কে", "ক্যারিয়ার", "প্রেস", "ব্লগ", "যোগাযোগ"],
      },
      courses: {
        title: "কোর্সসমূহ",
        links: [
          "IELTS",
          "স্পোকেন ইংলিশ",
          "ইমেইল মার্কেটিং",
          "কমপ্লিট গ্রামার",
          "একাডেমিক রাইটিং",
        ],
      },
      support: {
        title: "সাপোর্ট",
        links: [
          "হেল্প সেন্টার",
          "কমিউনিটি",
          "লাইভ চ্যাট",
          "FAQ",
          "সাপোর্ট যোগাযোগ",
        ],
      },
      legal: {
        title: "আইনি",
        links: [
          "প্রাইভেসি পলিসি",
          "সার্ভিস শর্তাবলী",
          "কুকি পলিসি",
          "রিফান্ড পলিসি",
        ],
      },
      contact: {
        title: "যোগাযোগের তথ্য",
        phone: "কল করুন: ১৬৯১০ (২৪x৭)",
        whatsapp: "হোয়াটসঅ্যাপ: +৮৮০১৮৯৬০১৬২৫২ (২৪x৭)",
        email: "ইমেইল: support@10minuteschool.com",
        address: "বাড়ি ২৫, রোড ৫, ধানমন্ডি, ঢাকা-১২০৫",
      },
      social: {
        title: "ফলো করুন",
        copyright: "© ২০২৪ ১০ মিনিট স্কুল। সকল অধিকার সংরক্ষিত।",
        description:
          "বাংলাদেশের বৃহত্তম অনলাইন শিক্ষা প্ল্যাটফর্ম যেখানে লক্ষ লক্ষ শিক্ষার্থী।",
      },
    },
  },
};

export function getTranslation(lang: "en" | "bn") {
  return translations[lang];
}
