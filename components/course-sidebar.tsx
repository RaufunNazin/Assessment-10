...
export function CourseSidebar({ ctaText, checklist, media, lang }: CourseSidebarProps) {
  ...
  const checklistTranslations = {
    en: {
      ...
      "১০টি লিখিত এবং ১০টি শিলনীর মক টেস্ট": "10+ assignments and practice tests",
    ...
    bn: {
      ...
      "10+ assignments and practice tests": "১০টি লিখিত এবং ১০টি শিলনীর মক টেস্ট",
      ...
      <CardContent className="p-0">
        ...
        <div className="p-6">
          <div className="text-center mb-6">
            <div className="mb-4">
              <div className="flex items-center justify-center gap-2 mb-2">
                <span className="text-lg text-slate-500 line-through">{prices.originalPrice}</span>
                <span className="bg-red-500 text-white px-2 py-1 rounded text-xs font-bold">{prices.discount}</span>
              </div>
              <div className="text-3xl font-bold text-slate-800">{prices.currentPrice}</div>
            </div>
            <Button
          ...
          <div className="space-y-3 md:space-y-4">
            <h3 className="font-semibold text-slate-800 text-sm md:text-base">{prices.whatYouGet}</h3>
            {checklist.map((item) => (
              <div
                key={item.id}
              ...
              >
                ...
                <span className="leading-relaxed">{translateChecklistItem(item.text)}</span>
              </div>
            ))}
          </div>
        </div>
