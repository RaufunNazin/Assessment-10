...
export default function ProductPage({ params }: PageProps) {
  ...
  const featuresSection = product.sections.find(
    (s: Section) => s.type === "features"
  );
  const pointersSection = product.sections.find(
    (s: Section) => s.type === "pointers"
  );
  const aboutSection = product.sections.find(
    (s: Section) => s.type === "about"
  );
  const testimonialsSection = product.sections.find(
    (s: Section) => s.type === "testimonials"
  );
        ...
        <div className="grid lg:grid-cols-3 gap-10">
          <div className="lg:col-span-2 space-y-10">
            {instructorSection && instructorSection.values.length > 0 && (
              <InstructorHeroCard
                instructor={instructorSection.values[0] as Instructor}
                lang={lang}
              />
            )}
            ...
            {featuresSection && featuresSection.values.length > 0 && (
              <CourseFeatures
                features={featuresSection.values as Feature[]}
                lang={lang}
              />
            )}
            ...
            {pointersSection && pointersSection.values.length > 0 && (
              <WhatYouLearn
                pointers={pointersSection.values as Pointer[]}
                lang={lang}
              />
            )}
            ...
            {aboutSection && aboutSection.values.length > 0 && (
              <CourseDetails
                aboutItems={aboutSection.values as AboutItem[]}
                lang={lang}
              />
            )}
            ...
            {testimonialsSection && testimonialsSection.values.length > 0 && (
              <ReviewsSection
                testimonials={testimonialsSection.values as Testimonial[]}
                lang={lang}
              />
            )}

            {faqSection && faqSection.values.length > 0 && (
              <FAQSection faqs={faqSection.values as FAQ[]} lang={lang} />
            )}
          </div>
